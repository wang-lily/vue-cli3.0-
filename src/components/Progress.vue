<template>
  <div>
    <div class="canvas" :id="canvasId">
      <canvas
        class="progress-canvas-bg"
        data-item="bg"
        :width="canvasWidth"
        :height="canvasWidth"
      ></canvas>
      <canvas
        class="progress-canvas-active"
        data-item="active"
        :width="canvasWidth"
        :height="canvasWidth"
      ></canvas>
...
export default {
  ...
  computed: {
    percent() {
      return ((this.options.nowVal / this.options.totalVal) * 100).toFixed(
  ...
  methods: {
    init() {
      this.backgroundCanvas.ele = document.querySelector(
        `#${this.canvasId} [data-item=bg]`
      )
      this.backgroundCanvas.context = this.backgroundCanvas.ele.getContext('2d')
      this.activeCanvas.ele = document.querySelector(
        `#${this.canvasId} [data-item=active]`
      )
    ...
    drawActive() {
      ...
      if (this.percent == 0) {
        this.activeCanvas.context.clearRect(
          0,
          0,
          this.canvasWidth,
          this.canvasHeight
    ...
    drawRectBg() {
      ...
      this.backgroundCanvas.context.moveTo(
        this.canvasLineWidth,
        this.canvasHeight / 2
      )
      this.backgroundCanvas.context.lineTo(
        this.canvasWidth - this.canvasLineWidth,
        this.canvasHeight / 2
      )
    ...
    drawRectActive() {
      ...
      this.activeCanvas.context.moveTo(
        this.canvasLineWidth,
        this.canvasHeight / 2
      )
      this.activeCanvas.context.lineTo(
        this.canvasLineWidth +
          ((this.canvasWidth - this.canvasLineWidth - this.canvasLineWidth) *
            this.percent) /
            100,
        this.canvasHeight / 2
      )
