<template>
  <div class="about" v-cloak>
    ...
    <div class="center">
      ...
      <Sign :width="sign.width" :height="sign.height" />
    </div>
    <div class="img-box">
...
import imgURL from '../assets/imgs/test/lb16.png'
export default {
  metaInfo: {
    title: 'We Inc',
    meta: [
  ...
  computed: {
    percent() {
      return (
        (this.progress.options.nowVal / this.progress.options.totalVal) *
        100
      ).toFixed(this.progress.options.tofixed)
    }
  },
  mounted() {
    // this.changeProgress()
  ...
  methods: {
    ...
    upload(event) {
      ...
      axios
        .post(`/add_travel/test`, form, config)
        .then(res => {
          if (res.data.code === 1) {
